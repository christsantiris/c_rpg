...
static void draw_shield_column(Renderer *r, const GameState *g, const Item *item, const Item *equipped, int x, int y, const char *heading, int selected_column) {
    SDL_Color gold = {220, 180, 60, 255};
    SDL_Color white = {200, 200, 200, 255};
    SDL_Color dimmed = {105, 105, 120, 255};
    SDL_Color green = {80, 170, 90, 255};
    ...
    renderer_draw_text(r, line, x + 10, y + 47, defense_color,
        r->font_tiny);
    SDL_snprintf(line, sizeof(line), "BLOCK: %d%%", item->block_chance);
    renderer_draw_text(r, line, x + 10, y + 63, white, r->font_tiny);
    SDL_snprintf(line, sizeof(line), "DAMAGE BLOCKED: %d%%",
    ...
    renderer_draw_text(r, line, x + 10, y + 111,
        allowed || !selected_column ? dimmed : red, r->font_tiny);
}
...
void draw_shield_comparison(Renderer *r, const GameState *g, const Item *selected, const Item *equipped, int y) {
    ...
    int panel_x = cx - column_width - 8;
    int panel_width = column_width * 2 + 16;
    SDL_Rect border = {panel_x, y, panel_width, 132};
    SDL_Rect panel = {panel_x + 2, y + 2, panel_width - 4, 128};
    SDL_SetRenderDrawColor(r->sdl, 55, 52, 77, 255);
    SDL_RenderFillRect(r->sdl, &border);
    SDL_SetRenderDrawColor(r->sdl, 14, 14, 29, 255);
    SDL_RenderFillRect(r->sdl, &panel);
    SDL_SetRenderDrawColor(r->sdl, 55, 52, 77, 255);
    SDL_RenderDrawLine(r->sdl, cx, y + 2, cx, y + 129);
    draw_shield_column(r, g, selected, equipped, panel_x + 2, y,
        "SELECTED", 1);
    draw_shield_column(r, g, equipped, equipped, cx + 2, y,
        "CURRENTLY EQUIPPED", 0);
}
